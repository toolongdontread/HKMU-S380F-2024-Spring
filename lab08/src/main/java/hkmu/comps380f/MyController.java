...
public class MyController {
    @GetMapping("/")
    public String index() { return "viewSessionActivity"; }

    @GetMapping("/checkboxes")
    public String multiValueForm() { return "MultiValueForm"; }
    @PostMapping("/checkboxes")
    public String multiValueResult() { return "MultiValueResult"; }
    ...
    public String recordSessionActivity(HttpServletRequest request, HttpSession session) {
        ...
        @SuppressWarnings("unchecked") CopyOnWriteArrayList<PageVisit> visits = (CopyOnWriteArrayList<PageVisit>) session.getAttribute("activity");
        if (!visits.isEmpty()) { PageVisit last = visits.get(visits.size() - 1);
    ...
    @GetMapping("/shop")
    public String shop(HttpServletRequest request, HttpSession session) throws IOException {
    ...
    private String viewCart(HttpServletRequest req)
            throws IOException {
    ...
    private String browse(HttpServletRequest req) {
        req.setAttribute("products", this.products);
        return "browse";
    }
}
